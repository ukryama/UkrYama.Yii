...
{       
	...
	public function run()
	{
            ...
            if($this->_config['premoderate'] === true && $this->evaluateExpression($this->_config['isSuperuser']) === false) $criteria->compare('t.status', Comment::STATUS_APPROWED);
            else {
                $criteria->compare('t.status', Comment::STATUS_APPROWED);
                $criteria->compare('t.status', Comment::STATUS_NOT_APPROWED, true, 'OR');
            }
